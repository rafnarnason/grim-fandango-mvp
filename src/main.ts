...
if (app) {
  ...
  function setupScreen(screenIdx: number, entrySide?: 'left' | 'right') {
    ...
    if (screenIdx === 0) {
      ...
      groundMat = new THREE.MeshPhongMaterial({ map: stoneTexture });
    } else {
      groundMat = new THREE.MeshPhongMaterial({ map: grassTexture });
    }
    ...
    if (playerTarget) {
      ...
      if (dist > playerSpeed) {
        ...
        if (playerPath.length > 0) {
          ...
          // If target is item, show message
          if (player.position.distanceTo(item.position) < 0.3 && !showItemMessage) {
